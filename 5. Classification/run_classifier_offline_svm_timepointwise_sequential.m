...
path.save = '~/Desktop/rtfMRI Reports/13:4:18';
...
%                     detection_table = zeros([number_thresholds numel(t0)*2]);
                    ...
                    test_bold_debug = [];
                    clear test_svm_label test_svm_label_size test_svm_label_size_rest test_svm_label_size_prep
                    
                    for t = numel(t0)*train_test_ratio+1:numel(t0) %0:numel(t0)
                        clc
...
%                                 prep_test = mean(feature_vector(:,prep_index_t + index_shift)');
                                ...
                                if j<= numel(t0)*train_test_ratio
...
%                     plot((1:size(mean_dmn,2))+bold_shift,mean(mean_dmn))
                    %%
                    close all
%                     avg_bold(response,s,r,[zeros([1 tr(numel(t0)*train_test_ratio+1)]) ts_label zeros([1 size(matrix,2) - (numel(ts_label) + tr(1) - 1)])],-4,'r',sprintf('Trained on %d',numel(t0)*train_test_ratio))
                    avg_bold(response,s,r,[zeros([1 tr(numel(t0)*train_test_ratio+1)-1]) ts_label linspace(0,0,size(matrix,2)-t0(end))],tr_sec*index_shift,'r',sprintf('Trained on %d',numel(t0)*train_test_ratio))
                    saveas(gca,sprintf('%s/%s svm labels s%d r%d shift%d prep%d trained on %d.png',path.save,feature_title,s,r,bold_shift,prep_initial_shift,numel(t0)*train_test_ratio))
...
%                     clear test_bold_debug
                    ...
                    accuracy = [];
                    for t=numel(t0)*train_test_ratio+1:numel(t0)
                        accuracy = [accuracy mean([linspace(1,1,test_svm_label_size_rest(t)) linspace(2,2,test_svm_label_size_prep(t))] == test_svm_label{t}')];
                    end
                    
                    accuracy_total{s,r} = accuracy;
                    accuracy_mean(s,r) = mean(accuracy);
