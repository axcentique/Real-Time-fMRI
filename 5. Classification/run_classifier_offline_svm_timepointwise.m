...
%         feature_name{6} = 'DMN & SMN BOLD derivative multiplication';
        ...
        for f = 1:numel(feature)
            ...
            for number_selected_voxels = 5:5:15
...
%                                     prep_test = feature_vector(:,prep_index_t + index_shift)';
    ...
    %                     plot((1:size(mean_dmn,2))+bold_shift,mean(mean_dmn))
                        %%
%                         close all
%                         avg_bold(response,s,r,[zeros([1 tr(1) - 1]) ts_label zeros([1 size(matrix,2) - (numel(ts_label) + tr(1) - 1)])],tr_sec*index_shift,'r',sprintf('%s voxels selected %d',feature_title,number_selected_voxels))
%                         saveas(gca,sprintf('%s/%s svm labels s%d r%d shift%d prep%d voxels selected %d.png',path.save,feature_title,s,r,bold_shift,prep_initial_shift,number_selected_voxels))
    ...
    %                     clear test_bold_debug
                        ...
                        accuracy_total{s,r,number_selected_voxels/5} = accuracy;
                        accuracy_mean(s,r,number_selected_voxels/5) = mean(accuracy);
