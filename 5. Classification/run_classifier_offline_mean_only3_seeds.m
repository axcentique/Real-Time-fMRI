...
path.save = '~/Desktop/rtfMRI Reports/13:3:29';
path.seed = '~/Data/rtfMRI/August/temp/dmn_seed_masks';
...
%         path.zmap = sprintf('%ssubject%d_run%d_contrast1_exclude%d_mni_1mm_in_func.nii.gz',path.feat,s,r,s);
        path.dmn = sprintf('%s/subj%d_rest%d_seed%d.nii.gz',path.seed,s,rest_num,dmn_seed);
...
%         matrix = rtfMRI_volume2matrix(mask_volume(func_hipass.vol,thresholded_zmap_smn));
        matrix_hi = rtfMRI_volume2matrix(mask_volume(func_hipass.vol,dmn_map.vol));
...
%         feature_name{6} = 'DMN & SMN BOLD derivative multiplication';
        ...
        %             plot_line = feature_dmn{2};
                    plot_line = [zeros([1 size(matrix_hi,2)-size(plot_line,2)]) plot_line];
                    fig_line_color = 'b';

                    time_index = (0:tr_sec:tr_sec*(size(matrix_hi,2)-1))-bold_shift*tr_sec;
