...
%         feature_name{6} = 'DMN & SMN BOLD derivative multiplication';
        ...
        for f = 4%[2 4]%1:numel(feature)
            %%
            feature_vector = feature{f};
            index_vector = feature_index{f};
            feature_title = feature_name{f};
            index_shift = numel(index_vector) - size(matrix_hi,2);

            for number_selected_voxels = 15%:5:50
...
for s = [4 6 7 8 10]
    temp_subj = [];
    for r = 1:5
        temp_subj = [temp_subj; accuracy_rest{s,r,f,number_selected_voxels/5} accuracy_prep{s,r,f,number_selected_voxels/5}];
    end
    out_table_avg = [out_table_avg; mean(temp_subj)];
    out_table_range = [out_table_range; min(temp_subj) max(temp_subj)];
end

out_table_sens = [out_table_avg(:,1) out_table_range(:,[1 3])]
out_table_spec = [out_table_avg(:,2) out_table_range(:,[2 4])]
