...
filepath_masks_root = sprintf('Art_Connect_1/NewMasks/');

% path where to save the tables with correlation values 
savePath = '~/Desktop/aging/new_masks/mesh/';
...
%             end
            ...
            %%
            for other_reg = reg+1:regNum
                t=1;
                clear corMat orphanedVoxelIndex
                %%
                vxlRange = network.region{other_reg}';
...
%                     end
        ...
        %             end
                    clc
                    fprintf('Subject %d/%d, Region %d&%d, Progress: %f',s,numel(subj),reg,other_reg,i/numel(vxl))
                end
...
% !!!!!!!!! Correlation matrixes
            corMatDB{s,reg,other_reg} = corMat;
            
            z = 0.5*log((1 + corMat) ./ (1 - corMat));
            zscore{s,reg,other_reg} = sum(z(:) / length(find(z)));
            zcorr{s,reg,other_reg} = tanh(zscore{s,reg,other_reg});
%             corAverageDB{s,reg,t} = sum(corMat(:)) / length(find((corMat)));
...
% 
%         load(sprintf('%sPairCor_%d.mat',corrPath,subj(s)));
        save(sprintf('%sMesh_interregion.mat',savePath),'corMatDB');%,'corAverageDB');
...
%     csvwrite(filepath, squeeze(resultTable(:,:,degree)));
    ...
    for reg=1:regNum-1
        for other_reg=reg+1:regNum
            header = [header [reg; other_reg]];
            zscore_table = [zscore_table zscore{s,reg,other_reg}];
            zcorr_table = [zcorr_table zcorr{s,reg,other_reg}];
        end
    end
    %%
    zscore_final(s,:) = zscore_table;
    zcorr_final(s,:) = zcorr_table;
    
    filepath = sprintf('%s%d_zscore.csv',savePath,subj(s));
    csvwrite(filepath, squeeze([subj(s), zscore_table]));

    filepath = sprintf('%s%d_zcorr.csv',savePath,subj(s));
    csvwrite(filepath, squeeze([subj(s), zcorr_table]));
    
end

filepath = sprintf('%smesh_interregion_header.csv',savePath);
csvwrite(filepath, header);

filepath = sprintf('%smesh_interregion_zscore.csv',savePath);
csvwrite(filepath, squeeze([subj(:), zscore_final(:,:)]));

filepath = sprintf('%smesh_interregion_zcorr.csv',savePath);
csvwrite(filepath, squeeze([subj(:), zcorr_final(:,:)]));