...
% motor_folder = 'Blocks+.feat/';
motor_folder = 'Events+.feat/';
motor_nii_file = 'zstat1.nii.gz';
% motor_nii_file = 'zstat3.nii.gz';
% dmn_folder = 'Event';
% run tut_blocks
run tut_events
...
path.motor = [data_path motor_folder 'stats/' motor_nii_file];
% path.func = [data_path ica_folder 'filtered_func_data.nii.gz'];
path.func = [data_path motor_folder 'filtered_func_data.nii.gz'];


comp = MRIread(path.ICA.comp);
mask = MRIread(path.motor);
...
save_path = sprintf('~/Desktop/Output/CABI_data/%s_design/ICA/',motor_folder);
